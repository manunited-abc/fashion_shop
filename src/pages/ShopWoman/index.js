...
function ShopWoman() {
    ...
    function filterProduct(categoryId, page, numOfProduct, sort, min, max) {
        fetch(
            `http://localhost:8080/api/products/woman/${categoryId}/?page=${page}&numOfProduct=${numOfProduct}&sort=${sort}&min=${min}&max=${max}`,
        )
    ...
    function getNumberOfPage(categoryId) {
        fetch('http://localhost:8080/api/products/number-product/woman/' + categoryId)
            .then((response) => response.json())
            .then((data) => {
